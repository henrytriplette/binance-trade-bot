...
class CryptoState:
    ...
    def __init__(self):
        if os.path.isfile(self._coin_backup_file) and os.path.isfile(
            self._table_backup_file
        ):
            ...
            self.coin_table = dict(
                (
                    coin_entry,
                    dict(
                        (coin, 0) for coin in supported_coin_list if coin != coin_entry
                    ),
                )
